        ...
        public static void main(String[] args) {
                ...
                try {
                        ...
                        for (int i = 1; i < sorok.size(); i++) {
                                ...
                                lista.add(new AbcKosarlabdaLiga(t[0], t[1], Integer.parseInt(t[2]),
                                                Integer.parseInt(t[3]), t[4],
                                                LocalDate.parse(t[5])));

                        }
                } catch (IOException ex) {
                        System.getLogger(AbcKosarlabdaLiga.class.getName()).log(System.Logger.Level.ERROR,
                                        (String) null, ex);
                }
                ...
                String barcelonaiCsapatNeve = lista.stream()
                                .filter(x -> x.getHazai().contains("Barcelona")).map(x -> x.getHazai()).findFirst()
                                .get();
                ...
                lista.stream().filter(x -> x.getIdopont().isEqual(
                                datum)).forEach(x -> System.out.println(
                                                "\t" + x.getHazai() + "-" + x.getIdegen() + " (" + x.getHazaiPont()
                                                                + ":" + x.getIdegenPont()
                                                                + ")"));
